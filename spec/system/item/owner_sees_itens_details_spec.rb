...
describe 'Owner sees items details' do
  it 'from home page' do
    ...
    expect(page).to have_content 'Panquecas de Blueberry com Mel Orgânico'
    expect(page).to have_content 'Deliciosas panquecas de blueberry servidas com mel orgânico e uma seleção de frutas frescas.'
    expect(page).to have_content 'Categoria: Café da Manhã'
    expect(page).to have_content 'Preço: 29.90'
    expect(page).to have_content 'Nível de Picante: 0'
    expect(page).to have_content 'Adicionais: Frutas frescas - 3.00'
    expect(page).to have_content 'Opção: vegetariano'
  end

  it 'and returns to menus page' do
    pix = PaymentMethod.create!(method: 'PIX')
    credito = PaymentMethod.create!(method: 'Cartão de Crédito')
    debito = PaymentMethod.create!(method: 'Cartão de Débito')
    categoria = Category.create!(name: 'Café da Manhã')
