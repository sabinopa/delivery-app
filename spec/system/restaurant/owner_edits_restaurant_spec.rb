...
describe 'Owner edits restaurant' do
  ...
  it 'cant edit others restaurant' do
    pix = PaymentMethod.create!(method: 'PIX')
    credito = PaymentMethod.create!(method: 'Cartão de Crédito')
    debito = PaymentMethod.create!(method: 'Cartão de Débito')
    ...
    visit edit_restaurant_path(restaurante_2.id)

    expect(current_path).to eq root_path
    expect(page).to have_content 'Apenas o proprietário pode editar esse restaurante.'
  end
end